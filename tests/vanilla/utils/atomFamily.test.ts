...
it('should notify listeners when atoms are removed', () => {
  ...
  const eventCreate = listener.mock.calls[0]?.[0] as unknown as Event
  ...
  aFamily.remove(1)
  expect(listener).toHaveBeenCalledTimes(1)
  const eventRemove = listener.mock.calls[0]?.[0] as unknown as Event
...
it('should handle multi-store removal with custom comparator', () => {
  ...
  const aFamily = atomFamily(initializeAtom, (a, b) => a.id === b.id)
  const listener = vi.fn()
  const unsubscribe = aFamily.unstable_listen(listener)
