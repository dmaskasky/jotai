...
export function atomFamily<Param, AtomType extends Atom<unknown>>(
  ...
  const atomInitFunctions = new WeakMap<AtomType, AtomType['unstable_onInit']>()
  ...
  const createAtom = (param: Param) => {
    ...
    Object.defineProperty(newAtom, 'unstable_onInit', {
      get() {
        return (store: Store) => {
          const initFunction = atomInitFunctions.get(newAtom)
          initFunction?.(store)
          atomFamilyInit(store)
        }
      },
      set(initFn: AtomType['unstable_onInit']) {
        atomInitFunctions.set(newAtom, initFn)
      },
      configurable: false,
      enumerable: true,
    })
